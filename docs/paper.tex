...
\paragraph{Independent proof validation.}
An independent proof checker (\texttt{alethe\_proof\_checker.py}) validates
certificates at four levels: structural well-formedness, rule validity,
premise resolution, and SMT re-verification.
On 69~freshly generated certificates (23~patterns $\times$ 3~models):
\emph{SMT re-verification achieves 100\%} (95\% CI $[94.7\%, 100\%]$),
confirming all verdicts are correct.
Structural validation achieves 69.6\%; the gap reflects UNSAT proofs
where Z3's internal proof format does not perfectly map to Alethe step
syntax, but the underlying formulas are independently re-verified.
...
\paragraph{Rigorous evaluation (expanded).}
A rigorous evaluation on \textbf{40 adversarial snippets} across
\textbf{8~categories} (message passing, store buffering, lock-free,
GPU, kernel patterns, application, dependency patterns, coherence)
provides per-category breakdown with Wilson CIs, a confusion matrix
over pattern families, and calibration analysis
(\texttt{rigorous\_llm\_evaluation.py}).

With gpt-4.1-nano: 52.5\% exact-match (CI $[37.5\%, 67.1\%]$).
Best categories: kernel patterns (100\%), application (80\%).
Weakest: dependency patterns (0\%), GPU (20\%).
Expected Calibration Error (ECE) = 0.38; the model is overconfident
in the 0.8--1.0 confidence bin.
...
\begin{table}[ht]
\centering
\caption{Feature comparison with related tools (expanded).}
\label{tab:comparison}
\small
\begin{tabular}{lcccccc}
\toprule
\textbf{Feature} & \textbf{\litmus{}} & \textbf{herd7} & \textbf{Dart.}
& \textbf{GenMC} & \textbf{CDS.} & \textbf{RCMC} \\
\midrule
Scope & 140 pats & litmus & programs & programs & programs & programs \\
Models & 7 & 10+ & 8+ & RC11 & C11 & RC11 \\
GPU & \cmark & \xmark & \xmark & \xmark & \xmark & \xmark \\
Proofs & Alethe & \xmark & witness & trace & trace & trace \\
Fences & \cmark & \xmark & \cmark & \xmark & \xmark & \xmark \\
Install & pip & opam & Maven & LLVM & source & LLVM \\
CI/JSON & \cmark & \xmark & \xmark & \xmark & \xmark & \xmark \\
Time/check & $<$6\,ms & ${\sim}$10\,ms & 100--500\,ms & 50--200\,ms & varies & varies \\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{Empirical false positive quantification.}
To address the critique that ``conservative shared-variable composition renders
the compositionality theorem inapplicable to most real programs,'' we
quantify false positive rates empirically.
We generate 20~multi-pattern programs across 7~interaction categories
(flag sharing, data sharing, counter sharing, pointer sharing, mixed sharing,
benign sharing, transitive sharing) plus disjoint and fenced baselines,
analyze each on 3~CPU architectures (60 total analyses), and compare the
conservative compositional verdict against joint SMT ground truth
(\texttt{false\_positive\_empirical.py}).

\emph{Results:} 8.3\% overall false positive rate
(95\% Wilson CI $[3.6\%, 18.1\%]$).
False positives concentrate in flag-sharing (33.3\%) and counter-sharing
(22.2\%) interactions.
Data-sharing, pointer-sharing, benign-sharing, and fenced programs
achieve 0\% false positives.
This quantifies the cost of conservatism: in the worst interaction
categories, approximately one-third of safe programs are over-flagged,
but the overall rate is below 10\%.
