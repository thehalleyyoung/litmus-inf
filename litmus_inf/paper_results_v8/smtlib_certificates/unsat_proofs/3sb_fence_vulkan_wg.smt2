...
; Pattern: 3sb_fence
; Forbidden outcome: {"r0": 0, "r1": 0, "r2": 0, "r3": 0}
; ═══════════════════════════════════════════════════════════════════
...
(assert (=> (= rf_8 0) (< ts_init_w ts_8)))
(assert (=> (= rf_8 1) (< ts_9 ts_8)))
(assert (=> (= rf_11 0) (< ts_init_x ts_11)))
(assert (=> (= rf_11 1) (< ts_0 ts_11)))
(assert (< ts_init_w ts_9))
(assert (< ts_init_x ts_0))
(assert (< ts_init_y ts_3))
(assert (< ts_init_z ts_6))
(assert (=> (= rf_2 0) (< ts_2 ts_3)))
(assert (=> (= rf_5 0) (< ts_5 ts_6)))
(assert (=> (= rf_8 0) (< ts_8 ts_9)))
(assert (=> (= rf_11 0) (< ts_11 ts_0)))
(assert (and (or (= rf_2 0)) (or (= rf_5 0)) (or (= rf_8 0)) (or (= rf_11 0))))

(check-sat)

; ── UNSAT Proof Certificate ──
; Status: UNSAT — forbidden outcome UNREACHABLE under Vulkan-WG
; Unsat core size: 9 assertions (out of 57)
; Core assertions: track_37, track_forbidden, track_55, track_54, track_36, track_53, track_52, track_38, track_39
(get-unsat-core)
