...
;   e2: T0 Store x = 1
;   e3: T0 Fence [scope=workgroup]
;   e4: T0 Store y = 2
;   e5: T1 Load y → r0
;   e6: T1 Fence [scope=workgroup]
;   e7: T1 Load x → r1
...
(assert (=> (= rf_5 2) (< ts_4 ts_5)))
(assert (=> (= rf_7 0) (< ts_init_x ts_7)))
(assert (=> (= rf_7 1) (< ts_2 ts_7)))
(assert (< ts_init_x ts_2))
(assert (< ts_init_y ts_0))
(assert (< ts_init_y ts_4))
(assert (=> co_y_1_2 (< ts_0 ts_4)))
(assert (=> co_y_2_1 (< ts_4 ts_0)))
(assert (=> (= rf_5 0) (< ts_5 ts_0)))
(assert (=> (= rf_5 0) (< ts_5 ts_4)))
(assert (=> (and (= rf_5 1) co_y_1_2) (< ts_5 ts_4)))
(assert (=> (and (= rf_5 2) co_y_2_1) (< ts_5 ts_0)))
(assert (=> (= rf_7 0) (< ts_7 ts_2)))
(assert (and (or (= rf_5 2)) (or (= rf_7 0))))

(check-sat)

; ── UNSAT Proof Certificate ──
; Status: UNSAT — forbidden outcome UNREACHABLE under Vulkan-WG
; Unsat core size: 5 assertions (out of 44)
; Core assertions: track_forbidden, track_30, track_27, track_42, track_26
(get-unsat-core)
